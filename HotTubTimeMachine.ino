...
// Variables to slow down the checks whether for how frequently the heater can be changed (avoid over-cycling when adjusting temperature), or how quickly to re-check the button (helps debounce - prevent multiple perceived button clicks for a single click)
...
const long heaterCheckDelay = 10000;  // 10 seconds: interval at which to potentially change whether the heater is on or off (milliseconds)
unsigned long lastDebounceTime = 0;  // the last time the output pin was toggled
const long debounceDelay = 100;    // the debounce time; increase if the output flickers
unsigned long lastTempCheck = 0;  // the last time the output pin was toggled
const long tempCheckDelay = 1000;    // the debounce time; increase if the output flickers
...
{
  if (Is_It_Time(&lastTempCheck, &tempCheckDelay) == true)
  {
...
bool Is_It_Time(unsigned long *timeLastReset, long *interval)
{
  ...
  if ((elapsedTime >= *interval) || (*timeLastReset == 0)) // run whenever the arduino is powered on/reset, an then at the appropriate intervals
  {
    // set the reset time to now because it is time to run the analysis
    *timeLastReset = currentTimeMilliSec;
    return true;
  } else {
    return false;
  }
...
{
  if (Is_It_Time(&lastDebounceTime, &debounceDelay) == true)
  {
